...
namespace netcli {
  ...
  inline const char *toNamespace(YangDataType t) {
    ...
    case YangDataType::IetfRestconfSubscribedNotifications:
      return "urn:ietf:params:xml:ns:yang:ietf-restconf-subscribed-"
             "notifications";
  ...
  inline YangDataType fromNamespace(const std::string &ns) {
    ...
    static const unordered_map<string, YangDataType> m = {
        ...
        {"urn:ietf:params:xml:ns:yang:ietf-routing-policy",
         YangDataType::IetfRoutingPolicy},
        {"urn:ietf:params:xml:ns:yang:ietf-routing-types",
         YangDataType::IetfRoutingTypes},
    ...
    if (pos != string::npos && pos + 1 < ns.size()) {
      ...
      for (const auto &p : m) {
        auto urn = p.first;
        if (urn.size() >= suffix.size() &&
            urn.compare(urn.size() - suffix.size(), suffix.size(), suffix) == 0)
          return p.second;
      }
    }
