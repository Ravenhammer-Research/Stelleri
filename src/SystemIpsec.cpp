...
void SystemConfigurationManager::SaveIpsec(const IpsecConfig &t) const {
  if (t.name.empty())
    throw std::runtime_error("IpsecConfig has no interface name set");
...
std::vector<IpsecConfig> SystemConfigurationManager::GetIpsecInterfaces(
    const std::optional<VRFConfig> &vrf) const {
  auto bases = GetInterfaces(vrf);
  std::vector<IpsecConfig> out;
