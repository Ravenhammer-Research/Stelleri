...
std::string IpsecTableFormatter::format(const std::vector<InterfaceConfig> &interfaces) {
  if (interfaces.empty()) {
  ...
  for (const auto &ic : interfaces) {
    ...
    const IpsecInterfaceConfig *tptr = dynamic_cast<const IpsecInterfaceConfig *>(&ic);
    if (tptr) {
      if (tptr->source)
