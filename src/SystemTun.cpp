...
void SystemConfigurationManager::SaveTun(const TunConfig &t) const {
  if (t.name.empty())
    throw std::runtime_error("TunConfig has no interface name set");
...
std::vector<TunConfig> SystemConfigurationManager::GetTunInterfaces(
    const std::optional<VRFConfig> &vrf) const {
  auto bases = GetInterfaces(vrf);
  std::vector<TunConfig> out;
