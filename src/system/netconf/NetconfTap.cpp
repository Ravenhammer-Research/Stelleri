...
void NetconfConfigurationManager::CreateTap(const std::string &name) const {
  Client *c = Client::instance();
  if (!c)
    throw NetconfError(nullptr, YangContext());

  // TAP interfaces are created using the standard ietf-interfaces model
  // with type set to iana-if-type:tunnel (131) - same as TUN
  // The difference between TUN and TAP is in how they're configured at the OS
  // level
  ...
  // Create the tap interface entry
...
void NetconfConfigurationManager::SaveTap(const TapInterfaceConfig &tap) const {
  Client *c = Client::instance();
  if (!c)
    throw NetconfError(nullptr, YangContext());

  // Convert to IetfInterfaces for the base interface config
  // Note: TAP-specific settings would require a custom YANG model
  IetfInterfaces ii(tap);
  struct lyd_node *node = ii.toLydNode(c->session().yangContext());
  if (!node)
    throw std::runtime_error(
        "Failed to convert TapInterfaceConfig to YANG node");
