...
void NetconfConfigurationManager::CreateTun(const std::string &name) const {
  Client *c = Client::instance();
  if (!c)
    throw NetconfError(nullptr, YangContext());

  // TUN interfaces are created using the standard ietf-interfaces model
  ...
  // Create the tun interface entry
...
void NetconfConfigurationManager::SaveTun(const TunInterfaceConfig &tun) const {
  Client *c = Client::instance();
  if (!c)
    throw NetconfError(nullptr, YangContext());

  // Convert to IetfInterfaces for the base interface config
  // Note: TUN-specific settings (tunnel parameters) would require a custom YANG
  // model
  IetfInterfaces ii(tun);
  struct lyd_node *node = ii.toLydNode(c->session().yangContext());
  if (!node)
    throw std::runtime_error(
        "Failed to convert TunInterfaceConfig to YANG node");
...
std::vector<TunInterfaceConfig> NetconfConfigurationManager::GetTunInterfaces(
  ...
  // Filter bases to only tun interfaces
  std::vector<TunInterfaceConfig> out;
  for (const auto &base : bases) {
    if (base.isTap()) { // TAP/TUN are both type "tunnel" in ietf-interfaces
      TunInterfaceConfig tif(base);
      out.push_back(tif);
    }
  }
