...
void NetconfConfigurationManager::CreateOvpn(const std::string &name) const {
  Client *c = Client::instance();
  if (!c)
    throw NetconfError(nullptr, YangContext());

  // OVPN interfaces are created using the standard ietf-interfaces model
  // with type set to iana-if-type:openvpn (281)
  // Note: This uses the vendor-specific extension for OpenVPN
  ...
  // Create the ovpn interface entry
  ...
  // Set type to iana-if-type:openvpn
  ...
  if (lyd_new_path(nullptr, yctx.get(), type_path.c_str(),
                   "iana-if-type:openvpn", 0, nullptr) != LY_SUCCESS)
    throw YangError(yctx);
...
void NetconfConfigurationManager::SaveOvpn(
  ...
  // Note: OVPN-specific settings (tunnel parameters, crypto) would require a
  // custom YANG model
  IetfInterfaces ii(ovpn);
  struct lyd_node *node = ii.toLydNode(c->session().yangContext());
  if (!node)
    throw std::runtime_error(
        "Failed to convert OvpnInterfaceConfig to YANG node");
...
std::vector<OvpnInterfaceConfig> NetconfConfigurationManager::GetOvpnInterfaces(
  ...
  // Filter bases to only ovpn interfaces
  std::vector<OvpnInterfaceConfig> out;
  for (const auto &base : bases) {
    if (base.isOvpn()) {
      OvpnInterfaceConfig oif(base);
      out.push_back(oif);
    }
  }
