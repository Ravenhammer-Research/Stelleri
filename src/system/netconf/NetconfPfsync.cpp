...
void NetconfConfigurationManager::CreatePfsync(const std::string &name) const {
  Client *c = Client::instance();
  if (!c)
    throw NetconfError(nullptr, YangContext());

  // PFSYNC interfaces are created using the standard ietf-interfaces model
  // with type set to iana-if-type:tunnel (131)
  // Note: pfSync sync interface for firewall state synchronization
  ...
  // Create the pfsync interface entry
...
void NetconfConfigurationManager::SavePfsync(
  ...
  // Note: PFSYNC-specific settings (sync peer, syncif) would require a custom
  // YANG model
  IetfInterfaces ii(p);
  struct lyd_node *node = ii.toLydNode(c->session().yangContext());
  if (!node)
    throw std::runtime_error(
        "Failed to convert PfsyncInterfaceConfig to YANG node");
...
void NetconfConfigurationManager::DestroyPfsync(const std::string &name) const {
  Client *c = Client::instance();
  if (!c)
    throw NetconfError(nullptr, YangContext());

  // Delete the pfsync interface using ietf-interfaces model
  YangContext yctx = c->session().yangContext();
  if (!yctx.get())
    throw YangError(yctx);
...
NetconfConfigurationManager::GetPfsyncInterfaces(
  ...
  // Filter bases to only pfsync interfaces
  std::vector<PfsyncInterfaceConfig> out;
  for (const auto &base : bases) {
    if (base.isPfsync()) {
      PfsyncInterfaceConfig pif(base);
      out.push_back(pif);
    }
  }
