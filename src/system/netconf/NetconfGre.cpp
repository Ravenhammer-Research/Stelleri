...
void NetconfConfigurationManager::CreateGre(const std::string &name) const {
  Client *c = Client::instance();
  if (!c)
    throw NetconfError(nullptr, YangContext());

  // GRE interfaces are created using the standard ietf-interfaces model
  // with type set to iana-if-type:gre (132)
  ...
  // Create the gre interface entry
  ...
  // Set type to iana-if-type:gre
  std::string type_path = std::format(
      "/ietf-interfaces:interfaces/interface[name='{}']/type", name);
  if (lyd_new_path(nullptr, yctx.get(), type_path.c_str(), "iana-if-type:gre",
...
void NetconfConfigurationManager::SaveGre(const GreInterfaceConfig &gre) const {
  Client *c = Client::instance();
  if (!c)
    throw NetconfError(nullptr, YangContext());

  // Convert to IetfInterfaces for the base interface config
  // Note: GRE-specific settings (tunnel parameters) would require a custom YANG
  // model
  IetfInterfaces ii(gre);
  struct lyd_node *node = ii.toLydNode(c->session().yangContext());
  if (!node)
    throw std::runtime_error(
        "Failed to convert GreInterfaceConfig to YANG node");
...
std::vector<GreInterfaceConfig> NetconfConfigurationManager::GetGreInterfaces(
  ...
  // Filter bases to only gre interfaces
  std::vector<GreInterfaceConfig> out;
  for (const auto &base : bases) {
    if (base.isGre()) { // GIF/GRE are both type "tunnel" in ietf-interfaces
      GreInterfaceConfig gif(base);
      out.push_back(gif);
    }
  }
