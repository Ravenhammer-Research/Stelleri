...
void NetconfConfigurationManager::CreateIpsec(const std::string &name) const {
  Client *c = Client::instance();
  if (!c)
    throw NetconfError(nullptr, YangContext());

  // IPSEC interfaces are created using the standard ietf-interfaces model
  // with type set to iana-if-type:ipsec (234)
  // Note: IPSEC tunnel configuration may require ietf-ipsec YANG model
  ...
  // Create the ipsec interface entry
  ...
  // Set type to iana-if-type:ipsec
  std::string type_path = std::format(
      "/ietf-interfaces:interfaces/interface[name='{}']/type", name);
  if (lyd_new_path(nullptr, yctx.get(), type_path.c_str(), "iana-if-type:ipsec",
...
void NetconfConfigurationManager::SaveIpsec(
  ...
  // Note: IPSEC-specific settings (SPI, encryption, authentication) would
  // require ietf-ipsec YANG model
  IetfInterfaces ii(ipsec);
  struct lyd_node *node = ii.toLydNode(c->session().yangContext());
  if (!node)
    throw std::runtime_error(
        "Failed to convert IpsecInterfaceConfig to YANG node");
...
NetconfConfigurationManager::GetIpsecInterfaces(
  ...
  // Filter bases to only ipsec interfaces
  std::vector<IpsecInterfaceConfig> out;
  for (const auto &base : bases) {
    if (base.isIpsec()) {
      IpsecInterfaceConfig iif(base);
      out.push_back(iif);
    }
  }
