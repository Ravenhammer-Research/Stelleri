...
void NetconfConfigurationManager::CreateGif(const std::string &name) const {
  Client *c = Client::instance();
  if (!c)
    throw NetconfError(nullptr, YangContext());

  // GIF interfaces are created using the standard ietf-interfaces model
  // with type set to iana-if-type:gif (138)
  ...
  // Create the gif interface entry
  ...
  // Set type to iana-if-type:gif
  std::string type_path = std::format(
      "/ietf-interfaces:interfaces/interface[name='{}']/type", name);
  if (lyd_new_path(nullptr, yctx.get(), type_path.c_str(), "iana-if-type:gif",
...
void NetconfConfigurationManager::SaveGif(const GifInterfaceConfig &gif) const {
  Client *c = Client::instance();
  if (!c)
    throw NetconfError(nullptr, YangContext());

  // Convert to IetfInterfaces for the base interface config
  // Note: GIF-specific settings would require a custom YANG model
  IetfInterfaces ii(gif);
  struct lyd_node *node = ii.toLydNode(c->session().yangContext());
  if (!node)
    throw std::runtime_error(
        "Failed to convert GifInterfaceConfig to YANG node");
