...
void NetconfConfigurationManager::CreateLagg(const std::string &name) const {
  Client *c = Client::instance();
  if (!c)
    throw NetconfError(nullptr, YangContext());

  // LAGG interfaces are created using the standard ietf-interfaces model
  // with type set to iana-if-type:ethernetCsmacd (6) - same as regular Ethernet
  // The lagg protocol is a FreeBSD-specific attribute not in ietf-interfaces
  ...
  // Create the lagg interface entry
...
void NetconfConfigurationManager::SaveLagg(
  ...
  // Note: LAGG-specific settings (protocol, members) would require a custom
  // YANG model
  IetfInterfaces ii(lac);
  struct lyd_node *node = ii.toLydNode(c->session().yangContext());
  if (!node)
    throw std::runtime_error(
        "Failed to convert LaggInterfaceConfig to YANG node");
...
std::vector<LaggInterfaceConfig> NetconfConfigurationManager::GetLaggInterfaces(
  ...
  // Filter bases to only lagg interfaces
  std::vector<LaggInterfaceConfig> out;
  for (const auto &base : bases) {
    if (base.isLagg()) {
      LaggInterfaceConfig lac(base);
      out.push_back(lac);
    }
  }
