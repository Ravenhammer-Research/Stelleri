#include "IpsecConfig.hpp"
#include "ConfigurationManager.hpp"
#include "InterfaceType.hpp"

void IpsecConfig::save(ConfigurationManager &mgr) const { mgr.SaveIpsec(*this); }

void IpsecConfig::create(ConfigurationManager &mgr) const { mgr.CreateIpsec(name); }

IpsecConfig::IpsecConfig(const InterfaceConfig &base) {
  name = base.name;
  type = InterfaceType::IPsec;
  if (base.address)
    address = base.address->clone();
  aliases.clear();
  for (const auto &a : base.aliases) {
    if (a)
      aliases.emplace_back(a->clone());
    else
      aliases.emplace_back(nullptr);
  }
  if (base.vrf)
    vrf = std::make_unique<VRFConfig>(*base.vrf);
  flags = base.flags;
  groups = base.groups;
  mtu = base.mtu;
}

IpsecConfig::IpsecConfig(const InterfaceConfig &base,
                         std::unique_ptr<IPAddress> source,
                         std::unique_ptr<IPAddress> destination)
    : IpsecConfig(base) {
  this->source = std::move(source);
  this->destination = std::move(destination);
}
