...
VirtualTableFormatter::format(const std::vector<ConfigData> &interfaces) const {
  ...
  // Columns: peer_a | VRF | MTU | Status | Flags | peer_b | VRF | MTU | Status | Flags
  ...
  // Build a map of pairs keyed by base name (strip trailing a/b when applicable)
  struct PairInfo {
  ...
  for (const auto &kv : pairs) {
    ...
    auto format_side = [&](const std::optional<InterfaceConfig> &opt) {
      ...
      std::string vrf = (ii.vrf && ii.vrf->table.has_value()) ? std::to_string(*ii.vrf->table) : std::string("-");
      ...
      std::string flags = ii.flags ? flagsToString(*ii.flags) : std::string("-");
    ...
    atf.addRow({name_a, a_side[0], a_side[1], a_side[2], a_side[3],
                name_b, b_side[0], b_side[1], b_side[2], b_side[3]});
  }
