...
std::string SingleInterfaceSummaryFormatter::format(const ConfigData &cd) const {
  if (!cd.iface)
  ...
  if (ic.lagg) {
    ...
    switch (ic.lagg->protocol) {
      case LaggProtocol::LACP: oss << "LACP"; break;
      case LaggProtocol::FAILOVER: oss << "Failover"; break;
      case LaggProtocol::LOADBALANCE: oss << "Load Balance"; break;
      case LaggProtocol::NONE: oss << "None"; break;
    }
    oss << "\n";
  ...
  if (ic.flags) {
    oss << "Flags:     ";
    if (*ic.flags & static_cast<uint32_t>(InterfaceFlag::UP)) oss << "UP ";
    if (*ic.flags & static_cast<uint32_t>(InterfaceFlag::BROADCAST)) oss << "BROADCAST ";
    if (*ic.flags & static_cast<uint32_t>(InterfaceFlag::LOOPBACK)) oss << "LOOPBACK ";
    if (*ic.flags & static_cast<uint32_t>(InterfaceFlag::POINTOPOINT)) oss << "POINTOPOINT ";
    if (*ic.flags & static_cast<uint32_t>(InterfaceFlag::RUNNING)) oss << "RUNNING ";
    if (*ic.flags & static_cast<uint32_t>(InterfaceFlag::MULTICAST)) oss << "MULTICAST";
    oss << "\n";
  }
